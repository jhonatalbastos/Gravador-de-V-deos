...
// Helper functions
const DB_NAME = 'LiturgiaStudioDB';
...
const removeFromDB = async (key: string) => {
    try {
        const db = await openDB();
        return new Promise<void>((resolve, reject) => {
            const transaction = db.transaction(STORE_NAME, 'readwrite');
...
export default function App() {
    ...
    const activeBgAudioElement = useRef<HTMLAudioElement | null>(null);
    
    const [backgroundAudioUrl, setBackgroundAudioUrl] = useState<string | null>(null);
    const [backgroundAudioName, setBackgroundAudioName] = useState<string>('');
    const [backgroundAudioVolume, setBackgroundAudioVolume] = useState<number>(0.15);
    ...
    useEffect(() => {
        ...
        if(s.bv) setBackgroundAudioVolume(parseFloat(s.bv));

        // Load background audio from persistence
        const loadPersistedBg = async () => {
            const data = await getFromDB('bg_audio');
            if (data && data.blob && data.name) {
                const url = URL.createObjectURL(data.blob);
                setBackgroundAudioUrl(url);
                setBackgroundAudioName(data.name);
            }
        };
        loadPersistedBg();
    }, []);
    ...
    const saveSettings = () => { 
        localStorage.setItem('v2_set', JSON.stringify({ 
            ts: titleFontSize, 
            ss: subtitleFontSize, 
            yp: textYPos,
            bv: backgroundAudioVolume
        })); 
            ...
            for (const [path, zipEntry] of Object.entries(contents.files)) {
                ...
                if (!numberMatch) {
                    if (isAudio) {
                        ...
                        setBackgroundAudioUrl(url);
                        setBackgroundAudioName(fileName);
                        await saveToDB('bg_audio', { blob, name: fileName });
                    }
                    continue;
                }
    ...
    const startFullVideoRecording = async () => {
        ...
        recorder.onstop = () => {
            ...
            if (bgAudio) {
                bgAudio.pause();
                activeBgAudioElement.current = null;
            }
    ...
    const stopRecording = () => { 
        ...
        if(activeBgAudioElement.current) {
            activeBgAudioElement.current.pause();
            activeBgAudioElement.current = null;
        }
        isPreviewing.current = true;
        isRecording.current = false;
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div className="sm:col-span-2 lg:col-span-3 border-t pt-4 mt-2">
                            <label className="block text-[10px] font-bold text-gray-500 uppercase mb-2 flex items-center gap-1">
                                <Music size={12} /> Som de Fundo (Trilha Sonora de Fundo para todos os Vídeos)
                            </label>
                            <div className="flex flex-col sm:flex-row items-center gap-4 bg-gray-50 p-3 rounded-xl border border-gray-100">
                                <div className="flex items-center gap-2 w-full sm:w-auto">
                                    <label className="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 px-4 py-2 rounded-lg text-xs font-bold shadow-sm active:scale-95 transition flex items-center gap-2 cursor-pointer whitespace-nowrap">
                                        <Music size={14} />
                                        <span>Upload Áudio de Fundo</span>
                                        <input type="file" className="hidden" accept="audio/*" onChange={async e => {
                                            ...
                                            if (file) {
                                                ...
                                                await saveToDB('bg_audio', { blob: file, name: file.name });
                                                showMsg("Áudio de fundo carregado!", "success");
                                            }
                                        }} />
                                    </label>
                                    {backgroundAudioUrl && (
                                        <button 
                                            onClick={async () => {
                                                ...
                                                setBackgroundAudioName('');
                                                await removeFromDB('bg_audio');
                                                showMsg("Áudio de fundo removido.", "info");
                                            }}
                                            className="text-red-500 hover:text-red-700 font-bold text-xs px-2 py-1"
                                            title="Remover som de fundo"
                                        >
                                            Excluir
                                        </button>
                                    )}
                                ...
                                <div className="flex-1 flex items-center gap-3 w-full">
                                    <span className="text-[10px] font-bold text-gray-500 truncate max-w-xs sm:max-w-md">
                                        {backgroundAudioName ? `Ficheiro: ${backgroundAudioName}` : 'Sem som de fundo ativo (apenas áudio dos blocos)'}
                                    </span>
                                    {backgroundAudioUrl && (
                                        <div className="flex items-center gap-2 ml-auto w-40 shrink-0">
